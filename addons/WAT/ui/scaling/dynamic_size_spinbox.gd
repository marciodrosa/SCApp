...
func _ready():
	...
	get_line_edit().connect("text_changed",Callable(self,"_on_text_edit_changed"))
	get_line_edit().connect("focus_exited",Callable(self,"_on_focus_exited"))
	...
	# This is needed or else the line edit starts unchecked unchecked-centered at startup
...
func _on_text_edit_changed(newText):
	...
	if not newText.is_empty() and not result:
		get_line_edit().text = oldText
		invalidInput = true
