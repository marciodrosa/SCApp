...
func _ready() -> void:
	custom_multiplayer.connect("connection_failed",Callable(self,"_on_connection_failed"))
...
@rpc func _on_tests_received_from_server(tests: Array, repeat: int, thread_count: int) -> void:
	var results: Array = await get_parent().run(tests, repeat, thread_count, self).completed
