var results: Array[TestModel.TestAssertResult] = []
...
const SignalAsserts: Script = preload("signal.gd")
...
func output(data: Dictionary) -> void:
	var assert_result = TestModel.TestAssertResult.new()
	if data.context != null and data.context != "":
		assert_result.message = "%s - %s" % [data.actual, data.context]
	else:
		assert_result.message = data.actual
	assert_result.passed = data.success
...
func signal_was_emitted(emitter, _signal, context: String = "") -> void:
	output(SignalAsserts.was_emitted(emitter, _signal, context))
	
func signal_was_emitted_x_times(emitter, _signal, times: int, context: String = "") -> void:
	output(SignalAsserts.was_emitted_x_times(emitter, _signal, times, context))

func signal_was_not_emitted(emitter, _signal: String, context: String = "") -> void:
	output(SignalAsserts.was_not_emitted(emitter, _signal, context))

func signal_was_emitted_with_arguments(emitter, _signal, arguments: Array, context: String = "") -> void:
	output(SignalAsserts.was_emitted_with_args(emitter, _signal, arguments, context))
